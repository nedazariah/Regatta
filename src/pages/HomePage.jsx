    ...
    <div>
      ...
      <section style={{
        ...
        <div style={{ position: 'relative', zIndex: 1, textAlign: 'center', width: '100%', maxWidth: 680, padding: '0 24px' }}>
          ...
          {/* Heading — reduced size so card is more centred */}
          <h1 style={{
            fontSize: 'clamp(28px, 5vw, 54px)', fontWeight: 700, lineHeight: 1.15,
          ...
          <p style={{ color: '#94a3b8', fontSize: 15, lineHeight: 1.7, marginBottom: 36, marginTop: 8 }}>
            144 modern rooms in the heart of the city, steps from Sarawak General Hospital,<br />
          ...
          <div style={{
              ...
              {[
                ...
                <>
                  <div key={label} style={{ padding: idx === 1 ? '0 0 0 20px' : '0 20px 0 0' }}>
                    <label style={{
                      display: 'block', color: '#D4A017', fontSize: 10, fontWeight: 700,
            ...
            <button
              ...
              style={{
                width: '100%', padding: '15px 0', background: '#2563eb', color: '#fff',
      ...
      {/* ── ABOUT ────────────────────────────────────────────── */}
      <section style={{ background: '#070c1a', padding: '80px 80px 64px' }}>
        <div style={{ maxWidth: 680, margin: '0 auto', textAlign: 'center' }}>
          <p style={{ color: '#D4A017', fontSize: 11, fontWeight: 700, letterSpacing: '0.15em', marginBottom: 14 }}>ABOUT REGATTA SUITES</p>
          <h2 style={{ fontSize: 38, fontWeight: 700, color: '#f1f5f9', lineHeight: 1.25, fontFamily: "'Playfair Display', Georgia, serif", marginBottom: 20 }}>
            Where Comfort Meets<br />Convenience in Kuching
          </h2>
          <p style={{ color: '#64748b', fontSize: 15, lineHeight: 1.8 }}>{HOTEL_INFO.description}</p>
        </div>
      </section>

      {/* ── EXPERIENCES ──────────────────────────────────────── */}
      <section style={{ background: '#070c1a', padding: '0 80px 96px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 40 }}>
          <div>
            <p style={{ color: '#D4A017', fontSize: 11, fontWeight: 700, letterSpacing: '0.15em', marginBottom: 12 }}>HOTEL FACILITIES</p>
            <h2 style={{ fontSize: 38, fontWeight: 700, color: '#f1f5f9', lineHeight: 1.2, fontFamily: "'Playfair Display', Georgia, serif" }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 24 }}>
          {EXPERIENCES.map((exp) => (
            ...
            >
              <div style={{ height: 200, overflow: 'hidden', background: '#0a1628' }}>
                <img
              ...
              <div style={{ padding: '20px 22px' }}>
                ...
                <h3 style={{ color: '#f1f5f9', fontSize: 19, fontWeight: 700, marginBottom: 10, fontFamily: "'Playfair Display', Georgia, serif" }}>{exp.title}</h3>
      ...
      {/* ── STATS ────────────────────────────────────────────── */}
      <section style={{ background: '#050a18', padding: '64px 80px', borderTop: '1px solid rgba(212,160,23,0.1)' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 40, textAlign: 'center' }}>
          {[
            ...
            <div key={stat.label}>
              <div style={{ fontSize: 40, fontWeight: 700, color: '#D4A017', fontFamily: "'Playfair Display', Georgia, serif" }}>{stat.number}</div>
              <div style={{ color: '#64748b', fontSize: 14, marginTop: 8 }}>{stat.label}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* ── LOCATION STRIP ───────────────────────────────────── */}
      <section style={{ background: '#070c1a', padding: '40px 80px', borderTop: '1px solid #0f172a' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            ...
            <div>
              ...
              <p style={{ color: '#64748b', fontSize: 13 }}>{HOTEL_INFO.address}</p>
            </div>
          </div>
